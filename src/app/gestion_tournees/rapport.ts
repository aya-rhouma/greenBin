...
export async function saveReportXml(body: any) {
  ...
  if (chef) {
    ...
    if (chef?.nom) employeesXml += `                <nom>${escapeXml(String(chef.nom))}</nom>\n`;
    if (chef?.prenom) employeesXml += `                <prenom>${escapeXml(String(chef.prenom))}</prenom>\n`;
  ...
  for (const id of trashIds) {
    if (!isNaN(id)) dechetsXml += `            <trashCan id="${id}" quantite="0"/>\n`;
  }
